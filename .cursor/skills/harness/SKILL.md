...
**MCP config**: `MCPRepository.get_mcp_config(user_id)` merges host `mcp.json` with that user's enabled Mongo connectors (`CompositeMCPRepository`). `AgentTaskRunner` passes `self._user_id`. File-backed servers stay read-only (`file:{name}` ids). Custom MCP CRUD lives in Settings → Connectors. The ChatBox **Connect apps** popover lists user Custom MCP switches (`connector.enabled`, same Mongo flag the toolkit already respects). Browse **Apps** only shows marketplace MCP with a public URL and no OAuth; those install via `POST /connectors/from-catalog` (idempotent `catalog_uid`). OAuth / BUILTIN / BYOK / Projects publish are omitted from the UI. Not part of the Plan-Act loop.
