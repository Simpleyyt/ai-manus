...
- The AI coding loop (scope → implement → verify → guard → sync → ship) is defined in AGENTS.md, with two project subagents in `.cursor/agents/`: `test-pyramid` (runs all verification layers) and `harness-reviewer` (read-only invariant review for `domain/services` diffs).
