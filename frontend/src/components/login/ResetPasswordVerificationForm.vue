...
const handleResendCode = async () => {
  ...
  try {
    ...
    await sendVerificationCode({ email: props.email })
    ...
    showErrorToast(t('Failed to resend verification code. Please try again.') + ': ' + errorMessage)
  }
}
...
onMounted(async () => {
  ...
  try {
    ...
    console.log('Initial verification code sent to:', props.email)
  } catch (error: any) {
    console.error('Failed to send initial verification code:', error)
    const errorMessage = error.response?.data?.message || error.message || 'Unknown error'
    showErrorToast(t('Failed to send verification code. Please try again.') + ': ' + errorMessage)
  }
