...
worker 容器复用 backend 镜像，在 compose 中额外添加一个 worker 服务即可：
...
- 每个 agent 任务运行期间会独占一个 worker 进程，`--concurrency` 决定可并行执行的 agent 会话数量。
- 也可以不通过容器直接启动 worker：`cd backend && uv run celery -A app.worker.celery_app worker --loglevel=INFO`。
