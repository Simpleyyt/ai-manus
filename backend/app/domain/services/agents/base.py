...
class BaseAgent(ABC):
    ...
    async def _parse_json(self, text: str) -> dict:
        """Parse JSON from LLM output (repaired by the LLM gateway if needed)."""
    ...
    def get_tool_schemas(self) -> List[dict]:
        ...
        return [tool.to_schema() for tool in self.get_tools()]

    async def invoke_tool(self, tool: Tool, tool_call: ToolCall) -> Tuple[LLMMessage, Any]:
        """Invoke specified tool, with retry mechanism.

        Returns a (tool_message, raw_result) tuple. raw_result is the tool's
        own return value (used for rich event rendering) and is None on failure.
        """
        retries = 0
        last_error = ""
        while retries <= self.max_retries:
            try:
                ...
                return LLMMessage.tool(tool_call.id, tool.name, content), raw_result
        ...
        return LLMMessage.tool(tool_call.id, tool.name, last_error), None
    ...
    async def execute(self, request: str, format: Optional[str] = None) -> AsyncGenerator[BaseEvent, None]:
        ...
        for _ in range(self.max_iterations):
            ...
            for tool_call in message.tool_calls:
                ...
                tool_message, raw_result = await self.invoke_tool(tool, tool_call)
    ...
    async def roll_back(self, message: Message):
        ...
        if function_name == "message_ask_user":
            self.memory.add_message(
                LLMMessage.tool(tool_call_id, function_name, message.message)
            )
        else:
            self.memory.roll_back()
        await self._repository.save_memory(self._agent_id, self.name, self.memory)
